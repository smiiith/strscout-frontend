-- Production Migration Script (PostgreSQL 15 Compatible - Safer Version)
...
-- 3. Add check constraints (with existence checks)
DO $$
BEGIN
...
-- 4. Create indexes (only if columns exist)
DO $$
BEGIN
    -- Create indexes for market_spy_runs
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_market_spy_runs_execution_time') THEN
        CREATE INDEX idx_market_spy_runs_execution_time ON public.market_spy_runs USING btree (execution_time_ms);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_market_spy_runs_profile_id') THEN
        CREATE INDEX idx_market_spy_runs_profile_id ON public.market_spy_runs USING btree (profile_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_market_spy_runs_scan_id') THEN
        CREATE INDEX idx_market_spy_runs_scan_id ON public.market_spy_runs USING btree (scan_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_market_spy_runs_started_at') THEN
        CREATE INDEX idx_market_spy_runs_started_at ON public.market_spy_runs USING btree (started_at);
    END IF;

    -- Create indexes for profiles (check if columns exist first)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'billing_type') THEN
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_billing_type') THEN
            CREATE INDEX idx_profiles_billing_type ON public.profiles USING btree (billing_type);
        END IF;
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'current_tier') THEN
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_current_tier') THEN
            CREATE INDEX idx_profiles_current_tier ON public.profiles USING btree (current_tier);
        END IF;
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'market_spy_listings_used')
       AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'market_spy_listings_limit') THEN
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_market_spy_usage') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'plan_id') THEN
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_plan_id') THEN
            CREATE INDEX idx_profiles_plan_id ON public.profiles USING btree (plan_id);
        END IF;
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'purchase_date') THEN
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_purchase_date') THEN
            CREATE INDEX idx_profiles_purchase_date ON public.profiles USING btree (purchase_date);
        END IF;
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'stripe_subscription_id') THEN
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_stripe_subscription_id') THEN
            CREATE INDEX idx_profiles_stripe_subscription_id ON public.profiles USING btree (stripe_subscription_id);
        END IF;
    END IF;
END $$;

-- 5. Create the update_updated_at_column function
...
-- 7. Add foreign key constraints (with existence checks)
DO $$
BEGIN
    -- comps foreign keys
    ...
    -- profiles foreign keys (only if plan_id exists and plans table exists)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'plan_id')
