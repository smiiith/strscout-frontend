...
export interface AmenitiesFeedbackStats {
...
export async function aggregateAmenitiesFeedbackData(): Promise<AmenitiesFeedbackStats> {
  ...
  for (const record of ratings) {
    ...
    const amenitiesData = ratingsData?.amenities;

    if (!amenitiesData) continue;

    const rating = amenitiesData.rating_number;
    if (rating) {
      totalRating += rating;
      ratingCount++;
    ...
    if (amenitiesData.feedback?.items) {
      for (const item of amenitiesData.feedback.items) {
        const theme = item.title;
        feedbackThemes.set(theme, (feedbackThemes.get(theme) || 0) + 1);
      }
    }

    if (amenitiesData.suggestions) {
      for (const suggestion of amenitiesData.suggestions) {
...
function getDefaultStats(): AmenitiesFeedbackStats {
  return {
    totalProperties: 0,
    averageRating: 0,
    ratingDistribution: {
