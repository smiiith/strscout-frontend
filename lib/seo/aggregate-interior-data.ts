...
export interface InteriorDesignFeedbackStats {
...
export async function aggregateInteriorDesignFeedbackData(): Promise<InteriorDesignFeedbackStats> {
  ...
  for (const record of ratings) {
    ...
    const interiorData = ratingsData?.interior_design;

    if (!interiorData) continue;

    const rating = interiorData.rating_number;
    if (rating) {
      totalRating += rating;
      ratingCount++;
    ...
    if (interiorData.feedback?.items) {
      for (const item of interiorData.feedback.items) {
        const theme = item.title;
        feedbackThemes.set(theme, (feedbackThemes.get(theme) || 0) + 1);
      }
    }

    if (interiorData.suggestions) {
      for (const suggestion of interiorData.suggestions) {
...
function getDefaultStats(): InteriorDesignFeedbackStats {
  return {
    totalProperties: 0,
    averageRating: 0,
    ratingDistribution: {
