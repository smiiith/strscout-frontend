...
const MarketScoutReportsPage = () => {
  return <MarketScoutReportsContent />;
};

const MarketScoutReportsContent = () => {
  ...
  const fetchComps = async () => {
    ...
    if (session && session.id) {
      ...
      try {
        ...
        const endpoint = `${process.env.NEXT_PUBLIC_API_ENDPOINT}/marketscout/comp-basis/${session.id}`;
    ...
    <div>
      {loading ? (
        ...
        <div className="min-h-[700px] py-6">
          <h1 className="text-3xl font-bold mt-6">Market Scout Reports</h1>

          <div className="space-y-6 w-full mt-6">
            <p className="w-2/3">
              View your Market Scout reports here. If one is in progress, use the
            ...
            {comps && <AddressList comps={comps} detailsPageUrl="/market-scout-details" />}
          </div>
        </div>
      )}
    </div>
...
export default MarketScoutReportsPage;
