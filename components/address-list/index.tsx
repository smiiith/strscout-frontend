...
export default function AddressList({ comps: comps }: AddressListProps) {
    ...
    const filteredAddresses = useMemo(() => {
        ...
        return comps.filter((address) => address.address.toLowerCase().includes(searchTerm.toLowerCase()))
    }, [comps, searchTerm])
        ...
        <div className="w-full space-y-6">
            ...
            {filteredAddresses.length === 0 && searchTerm.trim() && (
                <div className="text-center py-8">
                    <p className="text-muted-foreground">No addresses found matching "{searchTerm}"</p>
                </div>
            )}
        </div>
    )
