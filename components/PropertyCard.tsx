  ...
  const launchScan = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_ENDPOINT}/scan`,
        {
          property: property,
          profileId: profileId,
          headers: {
    ...
    <Card className="md:min-w-[500px] md:max-w-[500px] w-full border border-secondary-foreground">
      <CardHeader className="p-4 pb-0">
        <CardTitle className="flex gap-2">
          <div className="flex-grow md:text-2xl text-xl">{property.name}</div>
          <div
            className="cursor-pointer"
            title="Edit Property"
            onClick={() =>
              router.push(`/properties/add?property=${property.id}`)
            }
          >
            <PencilEdit02Icon />
          </div>
        ...
        <CardDescription>
          {property.listings.length > 0 && (
            <>
              Listed on: {property.listings[0]?.listed_on.toUpperCase()} and{" "}
              {property.listings[1]?.listed_on.toUpperCase()}
            </>
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid grid-cols-2 items-baseline gap-4 p-4 pt-2 text-gray-400">
            ...
            {isLoading ? (
              ...
              <>
                {property.lastscan && (
                  <div className="flex flex-row">
                    <div
                      className={`${property?.lastscan?.has_mismatch ? "text-red-500" : "text-green-500"} mr-1`}
                    >
                      {property?.lastscan?.has_mismatch ? (
                        <Alert02Icon />
                      ) : (
                        <CheckmarkCircle02Icon />
                      )}
                    ...
                    <div className="mx-1 mt-1">
                      <span className="font-bold">Last scan:</span>{" "}
                      {formatDate(property?.lastscan?.created_at)}
                      <div className="mt-2">
                        {property?.lastscan?.has_mismatch
                          ? "Issues Found"
                          : "No Issues"}
        ...
        <div className="col-span-1">Contact: {property.primary_contact}</div>
