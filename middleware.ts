...
export async function middleware(request: NextRequest) {
  ...
  if (
    isAuthenticated &&
    planProtectedRoutes[currentPath as keyof typeof planProtectedRoutes]
  ) {
    const requiredPlan =
      planProtectedRoutes[currentPath as keyof typeof planProtectedRoutes];
    ...
    if (!planCheck.hasAccess) {
      console.log(
        `Access denied to ${currentPath}: ${planCheck.reason}, user plan: ${planCheck.userPlan}`
      );
